...
class Read_Api:
    ...
    @staticmethod
    def get_probing_sine_from_signal(signal: np.ndarray, freq: float, cyc: int, decimation: int, threshold: float = 0.2) -> tuple[np.ndarray, int, int]:
        print("Probing for frequency: ", freq, " cyc: ", cyc, " decimation: ", decimation)
        for i in range(0, len(signal)):
            if signal[i] > threshold:
                print("Trigger at: ", i, " one block step: ", get_one_block_step(freq, cyc, decimation))
                return (signal[i:i+get_one_block_step(freq, cyc, decimation)], i, i+get_one_block_step(freq, cyc, decimation))
    ...
    def slide_correlate_through_one_block(probing: np.ndarray, signal: np.ndarray, start_block_index, end_block_index) -> np.ndarray:
        if start_block_index > end_block_index:
            print("Slide correlate through one block: start block index is greater than end block index")
            return np.zeros(0)
        ...
        for i in range(1, end_block_index-start_block_index):
            correlation = Read_Api.correlate_signal(probing[len(probing)-i:], signal[start_block_index:start_block_index+i])
    ...
    @staticmethod
    def correlate_through_one_block(probing: np.ndarray, signal: np.ndarray, start_block_index, end_block_index, samples, start_probing, end_probing, extremum_probe, plot=False) -> np.ndarray:
        if start_block_index > end_block_index:
            print("Slide correlate through one block: start block index is greater than end block index")
            return np.zeros(0)
        ...
        graph = np.empty(0)
        for i in range(-samples//2+1, min(samples//2+1, len(signal) - len(probing))):
            correlation = Read_Api.correlate_signal(probing, signal[start_block_index-i:start_block_index-i+len(probing)])
            center_value = correlation[int(len(correlation)//2)]
            graph = np.concatenate((graph, [center_value]))
    ...
    def get_block_indexes(signal, trigger, freq, cyc, decimation, index):
        ...
        return (start + (block_length * index), start + (block_length * index)+block_length)
