...
RGBABitmapImageReference *CreateRGBABitmapImageReference() {
  ...
  reference =
      (RGBABitmapImageReference *)malloc(sizeof(RGBABitmapImageReference));
...
double *ComputeGridLinePositions(size_t *returnArrayLength, double cMin,
  ...
  labels->stringArray =
      (StringReference **)malloc(sizeof(StringReference) * pNum);
...
ScatterPlotSettings *GetDefaultScatterPlotSettings() {
  ...
  settings->scatterPlotSeries =
      (ScatterPlotSeries **)malloc(sizeof(ScatterPlotSeries) * 0.0);
...
_Bool DrawScatterPlot(RGBABitmapImageReference *canvasReference, double width,
  ...
  settings->scatterPlotSeries =
      (ScatterPlotSeries **)malloc(sizeof(ScatterPlotSeries) * 1.0);
...
_Bool DrawScatterPlotFromSettings(RGBABitmapImageReference *canvasReference,
  ...
  if (success) {
    ...
    xLabelPriorities =
        (NumberArrayReference *)malloc(sizeof(NumberArrayReference));
    ...
    /* Labels */
    occupied =
        (Rectangle **)malloc(sizeof(Rectangle) * xLabels->stringArrayLength +
                             yLabels->stringArrayLength);
    ...
    /* Draw points */
    for (plot = 0.0; plot < settings->scatterPlotSeriesLength;
         plot = plot + 1.0) {
...
void ComputeBoundariesBasedOnSettings(ScatterPlotSettings *settings,
  ...
  if (!settings->autoBoundaries) {
    ...
    for (plot = 1.0; plot < settings->scatterPlotSeriesLength;
         plot = plot + 1.0) {
      sp = settings->scatterPlotSeries[(int)(plot)];
...
BarPlotSettings *GetDefaultBarPlotSettings() {
  ...
  settings->barPlotSeries =
      (BarPlotSeries **)malloc(sizeof(BarPlotSeries) * 0.0);
...
_Bool DrawBarPlot(RGBABitmapImageReference *canvasReference, double width,
  ...
  settings->barPlotSeries =
      (BarPlotSeries **)malloc(sizeof(BarPlotSeries) * 1.0);
...
_Bool DrawBarPlotFromSettings(RGBABitmapImageReference *canvasReference,
  ...
  if (success) {
    ...
    /* Labels */
    occupied =
        (Rectangle **)malloc(sizeof(Rectangle) * yLabels->stringArrayLength);
...
void TestMapping2(NumberReference *failures) {
  ...
  settings->scatterPlotSeries =
      (ScatterPlotSeries **)malloc(sizeof(ScatterPlotSeries) * 2.0);
  settings->scatterPlotSeriesLength = 2.0;
  settings->scatterPlotSeries[0] =
      (ScatterPlotSeries *)malloc(sizeof(ScatterPlotSeries));
  settings->scatterPlotSeries[0]->xs = xs;
  settings->scatterPlotSeries[0]->xsLength = xsLength;
  settings->scatterPlotSeries[0]->ys = ys;
  settings->scatterPlotSeries[0]->ysLength = ysLength;
...
void BarPlotExample(RGBABitmapImageReference *imageReference) {
  ...
  settings->barPlotSeries =
      (BarPlotSeries **)malloc(sizeof(BarPlotSeries) * 3.0);
...
void DrawLineBresenhamsAlgorithmThickPatterned(
  ...
  offset->numberValue =
      fmod(offset->numberValue + 1.0, patternLength * thickness);
...
CreateStringScientificNotationDecimalFromNumber(size_t *returnArrayLength,
                                                double decimal) {
  return CreateStringScientificNotationDecimalFromNumberAllOptions(
      returnArrayLength, decimal, false);
}
wchar_t *
CreateStringScientificNotationDecimalFromNumber15d2e(size_t *returnArrayLength,
                                                     double decimal) {
  return CreateStringScientificNotationDecimalFromNumberAllOptions(
      returnArrayLength, decimal, true);
}
wchar_t *CreateStringScientificNotationDecimalFromNumberAllOptions(
    size_t *returnArrayLength, double decimal, _Bool complete) {
  ...
  if (complete) {
    ...
    if (mantissaReference->stringLength == 1.0) {
      result =
          AppendString(&resultLength, result, resultLength, strparam(L"."));
      additional = additional - 1.0;
    }

    for (i = mantissaReference->stringLength; i < additional; i = i + 1.0) {
      result =
          AppendString(&resultLength, result, resultLength, strparam(L"0"));
    }
  }
...
_Bool CreateStringFromNumberWithCheck(double decimal, double base,
  ...
  if (decimal == 0.0) {
    ...
    characterReference =
        (CharacterReference *)malloc(sizeof(CharacterReference));
...
_Bool ExtractPartsFromNumberString(wchar_t *n, size_t nLength, double base,
  ...
  if (i < nLength) {
    ...
    success = ExtractPartsFromNumberStringFromSign(
        n, nLength, base, i, beforePoint, afterPoint, exponentIsPositive,
...
_Bool ExtractPartsFromNumberStringFromSign(wchar_t *n, size_t nLength,
  ...
  if (count >= 1.0) {
    ...
    if (i < nLength) {
      success = ExtractPartsFromNumberStringFromPointOrExponent(
          n, nLength, base, i, afterPoint, exponentIsPositive, exponent,
          errorMessages);
    } else {
...
_Bool ExtractPartsFromNumberStringFromPointOrExponent(
    wchar_t *n, size_t nLength, double base, double i,
  ...
  if (n[(int)(i)] == '.') {
    ...
    if (i < nLength) {
      ...
      if (count >= 1.0) {
        ...
        if (i < nLength) {
          success = ExtractPartsFromNumberStringFromExponent(
              n, nLength, base, i, exponentIsPositive, exponent, errorMessages);
        } else {
          exponent->numberArray = (double *)malloc(sizeof(double) * (0.0));
          exponent->numberArrayLength = 0.0;
          success = true;
...
_Bool ExtractPartsFromNumberStringFromExponent(
    wchar_t *n, size_t nLength, double base, double i,
      ...
      if (i < nLength) {
        ...
        if (count >= 1.0) {
          ...
          if (i == nLength) {
            ...
            success = false;
            errorMessages->string = L"There cannot be any characters past the "
                                    L"exponent of the number.";
            errorMessages->stringLength = wcslen(errorMessages->string);
          }
        } else {
          success = false;
...
_Bool aFillNumberArrayRange(double *a, size_t aLength, double value,
  ...
  if (from >= 0.0 && from <= aLength && to >= 0.0 && to <= aLength &&
      from <= to) {
    length = to - from;
    for (i = 0.0; i < length; i = i + 1.0) {
      a[(int)(from + i)] = value;
...
CreateStringArrayReferenceLengthValue(double length, wchar_t *value,
  ...
  ref = (StringArrayReference *)malloc(sizeof(StringArrayReference));
  ref->stringArray =
      (StringReference **)malloc(sizeof(StringReference) * length);
...
wchar_t *GetPixelFontData(size_t *returnArrayLength) {
  *returnArrayLength = wcslen(
      ...
      L"0011111110111101101111011111110011111100"
      "111111000111110001110000000000000000011111101110011111000011110000111100"
      "001111000011110000111100001111000011111001110111111000000000000000000000"
      "001100000011000000110000001100000011011111111110001111000011110000111110"
      "001101111111000000000000000011111100011101101111101111011011110000111100"
      "001111000011110000111100001101100110001111000000000000000000110000110110"
      "001100110011000110110000111101111111111000111100001111000011111000110111"
      "111100000000000000000111111011100111110000001100000011100000011111100000"
         "110000111100001111000011110000111111111000000000000000000000000000000"
         "000000000000000000000000011000000110000001100000011000000110000011101"
         "111111000000000000000000000000000000000000000000000000011111111100000"
         "011000000011111100000001100000011111111100000000000000000000000000000"
         "000000000000000000000011100001101100000011000000110000001100000011000"
         "011111100001100000011000000110000000000000000000000000001111110011000"
         "110110001101100011011000110110001101100011000000000000000000000000000"
         "000000000000000000000000110000011110000111100011001100110011011000011"
         "110000110000000000000000000000000000000000000000000000001100001111100"
         "111111111111101101111000011110000111100001100000000000000000000000000"
         "000000000000000000000011000011011001100011110000011000001111000110011"
         "011000011000000000000000000000000000000000000001100000110000001100000"
         "110000011000001111000110011001100110110000110000000000000000000000000"
         "000000000000000000000001111111100000110000011000001100000110000011000"
         "001111111100000000000000000000000000000000000000000000000011110000000"
         "110000001100000011000000111000000111100011100000110000001100000011000"
         "111100000001100000011000000110000001100000011000000110000001100000011"
         "000000110000001100000011000000110000001100000000000000000000000111100"
         "011000000110000001100000111000111100000011100000011000000110000001100"
         "000001111";
}
...
double *ConvertToPNG(size_t *returnArrayLength, RGBABitmapImage *image) {
  return ConvertToPNGWithOptions(returnArrayLength, image, 6.0, false, 0.0,
                                 0.001);
}
double *ConvertToPNGGrayscale(size_t *returnArrayLength,
                              RGBABitmapImage *image) {
  return ConvertToPNGWithOptions(returnArrayLength, image, 0.0, false, 0.0,
                                 0.001);
}
...
double *ConvertToPNGWithOptions(size_t *returnArrayLength,
                                RGBABitmapImage *image, double colorType,
  ...
  png->zlibStruct =
      ZLibCompressStaticHuffman(colorData, colorDataLength, compressionLevel);
...
double *PNGSerializeChunks(size_t *returnArrayLength, PNGImage *png) {
  ...
  length = png->signatureLength + 12.0 + PNGHeaderLength() + 12.0 +
  ...
  Write4BytesBE(data, dataLength,
                CRC32OfInterval(data, dataLength,
  ...
  for (i = 0.0; i < png->zlibStruct->CompressedDataBlocksLength; i = i + 1.0) {
    WriteByte(data, dataLength, png->zlibStruct->CompressedDataBlocks[(int)(i)],
              position);
  }
  ...
  Write4BytesBE(
      data, dataLength,
      CRC32OfInterval(data, dataLength, position->numberValue - 4.0, 4.0),
      position);
...
double *GetPNGColorDataGreyscale(size_t *returnArrayLength,
                                 RGBABitmapImage *image) {
  double *colordata;
  size_t colordataLength;
  double length, x, y, next;
...
Chunk **PNGReadChunks(size_t *returnArrayLength, double *data,
                      size_t dataLength, NumberReference *position) {
  _Bool done;
  double prepos;
  double chunks;
  Chunk *c;
...
Chunk *PNGReadChunk(double *data, size_t dataLength,
                    NumberReference *position) {
  Chunk *c;
...
_Bool SubstringWithCheck(wchar_t *string, size_t stringLength, double from,
  ...
  if (from >= 0.0 && from <= stringLength && to >= 0.0 && to <= stringLength &&
      from <= to) {
...
_Bool IndexOfString(wchar_t *string, size_t stringLength, wchar_t *substring,
  ...
  found = false;
  for (i = 0.0; i < stringLength - substringLength + 1.0 && !found;
       i = i + 1.0) {
...
wchar_t *ReplaceString(size_t *returnArrayLength, wchar_t *string,
  ...
  for (i = 0.0; i < stringLength;) {
    ...
    if (success && toReplaceLength > 0.0) {
      result = ConcatenateString(&resultLength, result, resultLength,
                                 replaceWith, replaceWithLength);
      i = i + toReplaceLength;
    } else {
      result = ConcatenateCharacter(&resultLength, result, resultLength,
                                    string[(int)(i)]);
      i = i + 1.0;
    }
  }
...
wchar_t *ReplaceCharacter(size_t *returnArrayLength, wchar_t *string,
                          size_t stringLength, wchar_t toReplace,
                          wchar_t replaceWith) {
  wchar_t *result;
  size_t resultLength;
...
wchar_t *Trim(size_t *returnArrayLength, wchar_t *string, size_t stringLength) {
  ...
  firstNonWhitespaceFound = false;
  for (i = stringLength - 1.0; i >= 0.0 && !firstNonWhitespaceFound;
       i = i - 1.0) {
    if (charIsWhiteSpace(string[(int)(i)])) {
...
_Bool EndsWith(wchar_t *string, size_t stringLength, wchar_t *end,
  ...
  if (stringLength >= endLength) {
    endsWithString = SubstringEquals(string, stringLength,
                                     stringLength - endLength, end, endLength);
  }
...
double *ReadXbytes(size_t *returnArrayLength, double *data, size_t dataLength,
                   NumberReference *position, double length) {
  double *r;
  size_t rLength;
  double i;
...
double Read4bytesBE(double *data, size_t dataLength,
                    NumberReference *position) {
  double r;
...
double Read2bytesBE(double *data, size_t dataLength,
                    NumberReference *position) {
  double r;
...
double Read4bytesLE(double *data, size_t dataLength,
                    NumberReference *position) {
  double r;
...
void WriteByte(double *data, size_t dataLength, double b,
               NumberReference *position) {
  data[(int)(position->numberValue)] = b;
  position->numberValue = position->numberValue + 1.0;
}
void Write2BytesLE(double *data, size_t dataLength, double b,
                   NumberReference *position) {
  data[(int)(position->numberValue)] = Round(fmod(b, pow(2.0, 8.0)));
  position->numberValue = position->numberValue + 1.0;
  data[(int)(position->numberValue)] =
      fmod(floor(b / pow(2.0, 8.0)), pow(2.0, 8.0));
  position->numberValue = position->numberValue + 1.0;
}
void Write4BytesLE(double *data, size_t dataLength, double b,
                   NumberReference *position) {
  data[(int)(position->numberValue)] = Round(fmod(b, pow(2.0, 8.0)));
  position->numberValue = position->numberValue + 1.0;
  data[(int)(position->numberValue)] =
...
void Write2BytesBE(double *data, size_t dataLength, double b,
                   NumberReference *position) {
  data[(int)(position->numberValue)] =
      fmod(floor(b / pow(2.0, 8.0)), pow(2.0, 8.0));
  position->numberValue = position->numberValue + 1.0;
...
void Write4BytesBE(double *data, size_t dataLength, double b,
                   NumberReference *position) {
  data[(int)(position->numberValue)] =
      fmod(floor(b / pow(2.0, 24.0)), pow(2.0, 8.0));
  position->numberValue = position->numberValue + 1.0;
...
void WriteStringBytes(double *data, size_t dataLength, wchar_t *cs,
                      size_t csLength, NumberReference *position) {
  double i, v;
...
double UpdateCRC32(double crc, double *buf, size_t bufLength, double *crc_table,
...
double CRC32OfInterval(double *data, size_t dataLength, double from,
                       double length) {
  double *crcBase;
...
ZLIBStruct *ZLibCompressNoCompression(double *data, size_t dataLength) {
  ...
  zlibStruct->FLG = 1.0;
  zlibStruct->CompressedDataBlocks = DeflateDataNoCompression(
      &zlibStruct->CompressedDataBlocksLength, data, dataLength);
  zlibStruct->Adler32CheckValue = ComputeAdler32(data, dataLength);

  return zlibStruct;
}
ZLIBStruct *ZLibCompressStaticHuffman(double *data, size_t dataLength,
                                      double level) {
  ZLIBStruct *zlibStruct;
  ...
  zlibStruct->CMF = 120.0;
  zlibStruct->FLG = 1.0;
  zlibStruct->CompressedDataBlocks = DeflateDataStaticHuffman(
      &zlibStruct->CompressedDataBlocksLength, data, dataLength, level);
  zlibStruct->Adler32CheckValue = ComputeAdler32(data, dataLength);
...
StringReference **AddString(size_t *returnArrayLength, StringReference **list,
  ...
  newlist =
      (StringReference **)malloc(sizeof(StringReference) * listLength + 1.0);
  newlistLength = listLength + 1.0;
...
void LinkedListAddString(LinkedListStrings *ll, wchar_t *value,
  ...
  ll->last->next =
      (LinkedListNodeStrings *)malloc(sizeof(LinkedListNodeStrings));
...
void LinkedListAddNumber(LinkedListNumbers *ll, double value) {
  ...
  ll->last->value = value;
  ll->last->next =
      (LinkedListNodeNumbers *)malloc(sizeof(LinkedListNodeNumbers));
  ll->last->next->end = true;
  ll->last = ll->last->next;
}
...
LinkedListCharacters *CreateLinkedListCharacter() {
  ...
  ll->first =
      (LinkedListNodeCharacters *)malloc(sizeof(LinkedListNodeCharacters));
  ll->last = ll->first;
  ll->last->end = true;
...
double *DeflateDataStaticHuffman(size_t *returnArrayLength, double *data,
                                 size_t dataLength, double level) {
  double *bytes;
  size_t bytesLength;
  ...
  for (i = 0.0; i < dataLength;) {
    FindMatch(data, dataLength, i, distanceReference, lengthReference, match,
              level);
    ...
    if (!match->booleanValue) {
      GetDeflateStaticHuffmanCode(data[(int)(i)], code, length,
                                  bitReverseLookupTable,
                                  bitReverseLookupTableLength);
      AppendBitsToBytesRight(bytes, bytesLength, currentBit, code->numberValue,
                             length->numberValue);
      i = i + 1.0;
    } else {
      ...
      AppendBitsToBytesRight(bytes, bytesLength, currentBit, code->numberValue,
                             length->numberValue);
      AppendBitsToBytesRight(bytes, bytesLength, currentBit,
                             lengthAddition->numberValue,
                             lengthAdditionLength->numberValue);
      AppendBitsToBytesRight(bytes, bytesLength, currentBit,
                             distanceCode->numberValue, 5.0);
      AppendBitsToBytesRight(bytes, bytesLength, currentBit,
  ...
  aCopyNumberArrayRange(bytes, bytesLength, 0.0,
                        ceil(currentBit->numberValue / 8.0), copy);
  free(bytes);
...
void FindMatch(double *data, size_t dataLength, double pos,
               NumberReference *distanceReference,
...
double *DeflateDataNoCompression(size_t *returnArrayLength, double *data,
                                 size_t dataLength) {
  double *deflated;
  ...
  deflated =
      (double *)malloc(sizeof(double) * ((1.0 + 4.0) * blocks + dataLength));
  ...
  for (block = 0.0; block < blocks; block = block + 1.0) {
    ...
    for (i = 0.0; i < blocklength; i = i + 1.0) {
      WriteByte(deflated, deflatedLength, data[(int)(block * maxblocksize + i)],
                position);
    }
  }
...
void AppendBitsToBytesLeft(double *bytes, size_t bytesLength,
                           NumberReference *nextbit, double data,
                           double length) {
...
void AppendBitsToBytesRight(double *bytes, size_t bytesLength,
                            NumberReference *nextbit, double data,
                            double length) {
